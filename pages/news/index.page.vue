<template>
	<div>
		<template v-if="importantNews">
			<SliderThree
				v-if="importantNews.list.length !== 0"
				v-slot="slide"
				v-bind="importantNews"
				class="border-none"
			>
		...
		<div class="container-cancel">
			<hr class="mt-4 border-t-2 text-red-100" />
		</div>

		<section>
			<div class="py-6">
				<h1 class="mb-4 font-serif text-[34px]">
					<b class="font-semibold text-red-100">{{ pageHead.titleHead }}</b>
					{{ pageHead.titleTail }}
				</h1>
				<p>
					{{ pageHead.subtitle }}
			...
			<div
				v-if="heroItems"
				class="mb-14 mt-11 flex flex-wrap gap-2 lg:flex-nowrap"
			>
				...
				<div v-if="columnList" class="row">
					<div class="lg:col-6">
						<template v-if="columnList.left">
							<BlockHeader :title="columnList.left.title" class="mb-6" />
							<CardThree
					...
					<div class="lg:col-6">
						<template v-if="columnList.right">
							<BlockHeader :title="columnList.right.title" class="mb-6" />
							<CardThree
...
import SliderThree from '~/components/slider-three/slider-three.component.vue';
...
import { NewsDocument, NewsQuery } from './index.query';

const { t } = useI18n();
const userInfo = useUserInfo();
const { data, error } = await useAsyncQuery(NewsDocument);
...
type Pages = NonNullable<NewsQuery['siteManagerNewsHomePages']>;
const page = computed<Pages['data'][number] | undefined>(() => {
	const maybePages = data.value?.siteManagerNewsHomePages?.data;
...
const pageHead = computed(() => {
	const head = page.value?.attributes;
	const [titleHead, ...titleTail] = (head?.title ?? 'News').split(' ');

	return {
		titleHead: titleHead,
		titleTail: titleTail.join(' '),
...
const importantNews = computed(() => {
	const flashNews = page.value?.attributes?.flashNews;
	if (!flashNews?.active) return undefined;

	const tags = (flashNews?.contentList?.data ?? []).filter(isValidEntity);
	const contents = tags
		.flatMap((tag) => tag.attributes.contents?.data)
		.filter(Boolean)
		.filter(isValidEntity)
		.slice(0, flashNews?.numberOfItems ?? 6);
