<template>
	...
	>
		...
		>
			<div
				class="absolute inset-0 bg-white-100"
			...
			>
				...
				>
					<div>
						<div
							class="flex w-full items-center justify-between gap-5 border-b border-grey-100 px-5 py-4 text-red-100"
						>
							<a href="/">
						...
						<button
							class="flex w-full items-center gap-3 bg-grey-200 px-5 py-3"
							@click="$emit('toggleSearchModal')"
						>
							<IconSearch class="h-5 w-5" />
							<h3 class="text-sm font-semibold">{{ t('shared.search') }}</h3>
						</button>
						<div class="border-b border-grey-100">
							<component
								...
								:key="category.title"
								class="flex w-full items-center justify-between px-5 py-3 text-sm font-semibold tracking-wide duration-75 hover:bg-red-100 hover:text-white-100"
								:class="{
						...
						<div class="border-b border-grey-100">
							<a
								:href="magazineUrl"
								class="block w-full px-5 py-3 text-left text-sm font-semibold tracking-wide duration-75 hover:bg-red-100 hover:text-white-100"
							>
								{{ t('shared.magazine') }}
							</a>
							<button
					...
					<div>
						<a
							v-if="!user"
							:href="signinUrl"
							class="flex w-full items-center justify-center gap-2 bg-red-100 py-3 text-white-100 duration-75 hover:bg-red-200"
						>
						...
						<button
							v-if="user"
							class="flex w-full items-center justify-between gap-2 bg-red-100 px-5 py-3 text-white-100 duration-75 hover:bg-red-200"
						...
						<div class="bg mx-5 flex flex-wrap justify-between gap-2 py-3">
							<a
				...
				>
					<div
						v-if="activeCategory?.items || userMenuOpen"
						class="absolute left-0 top-0 h-full w-full bg-white-100"
					>
						<div v-if="activeCategory?.items">
							<div
								class="flex w-full items-center justify-between gap-5 border-b border-grey-100 px-5 py-4"
							>
								<button @click="activeCategory = undefined">
									<IconChevronLeft class="h-5 w-5" />
								</button>
								<a
									class="block w-full px-5 text-center font-serif text-lg font-bold tracking-wide"
						...
						<div v-if="userMenuOpen">
							...
							<SidebarUserMenu @sign-out="signOut" />
						</div>
					</div>
				</transition>
			</div>
